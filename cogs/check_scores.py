        ...
        for i, beatmap in enumerate(beatmaps):
            if i >= 0:
                ...
                for i, friend in enumerate(friends):
                    ...
                    scores = await self.database.get_user_beatmap_plays_with_zeros(friend[1], beatmap[0])
                    if scores:
                        for score in scores:
                            check = True
                            for snipe in snipes:
                        ...
                        await asyncio.sleep(0.001)
                        friend_play = await self.osu.get_score_data(beatmap[0], friend[1])
                        if friend_play:
                            print(f"adding friend score for {friend_play['score']['user']['username']}")
                            await self.database.add_score(friend[1], beatmap[0], friend_play['score']['score'], 0)
                        else:
